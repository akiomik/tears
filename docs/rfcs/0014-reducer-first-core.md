...
| 9 | RFC 0006 | vocabulary amendment | INV-L13 schema: `shared_pending` reads as the data lane's residual occupancy; `channel`'s value domain becomes the single value `"data"`; gauge kind counts map (`unkeyed_commands` = anonymous runs, `keyed_commands` = keyed runs); firing conditions unchanged. The field names are **re-read, not renamed**, and deliberately so — the opposite of row 2's decision for the configuration control, because the two faces fail differently: a renamed config field fails the build that uses it, which is how row 2's widened meaning reaches its callers, while a renamed telemetry field breaks dashboards, alert rules and log parsers **silently**, off the compiler's path entirely. Schema stability is what an observability consumer depends on, so this row amends meanings and the `channel` value domain and leaves every field name where it is |
